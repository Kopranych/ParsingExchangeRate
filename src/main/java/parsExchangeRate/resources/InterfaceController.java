...
public class InterfaceController implements ControllerExchangeRate {
	...
	public void getActualExchangeRate() {
		...
		try {
			...
			} else if (exchangeRatePreview.getUsd() < currencyEngine.getExchanger().getUSD()) {
	...
	private void getChart() {
		try {
			...
			ArrayList<ExchangeRateDataSet> list = (ArrayList<ExchangeRateDataSet>) service.getExchangeRateListRange(tempId - 45, tempId);
